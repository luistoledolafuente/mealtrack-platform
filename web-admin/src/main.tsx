...
function App() {
  ...
  return <div className="shell">
      ...
      <section id="support" className="support-section" aria-labelledby="support-title">
        ...
        {!selectedRestaurant ? <div className="support-empty">Selecciona un restaurante para gestionar sus planes, alumnos, administradores y suscripciones.</div> : <>
          ...
          {supportLoading ? <div className="state">Cargando información operativa…</div> : support && <div className="support-grid">
            ...
            <SupportPanel title="Suscripciones" description="Asigna o renueva una pensión para un estudiante existente." count={support.subscriptions.length} action="Asignar pensión" onAction={() => setShowSubscription(true)}><SubscriptionList subscriptions={support.subscriptions} /></SupportPanel>
          </div>}
        </>}
      </section>
    </main>
    ...
    {selectedRestaurant && showSubscription && <SubscriptionDialog restaurant={selectedRestaurant} students={students} plans={support?.plans || []} token={session.token} onClose={() => setShowSubscription(false)} onSaved={() => { setShowSubscription(false); void loadSupport(); }} />}
  </div>;
}
...
function SubscriptionDialog({ restaurant, students, plans, token, onClose, onSaved }: { restaurant: Restaurant; students: User[]; plans: MealPlan[]; token: string; onClose: () => void; onSaved: () => void }) {
  const [studentUserId, setStudentUserId] = useState(''); const [mealPlanId, setMealPlanId] = useState(''); const [startDate, setStartDate] = useState(new Date().toISOString().slice(0, 10)); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null);
  const activePlans = plans.filter((plan) => plan.isActive);
  const save = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(null); try { await request<Subscription>('/subscriptions', { method: 'POST', body: JSON.stringify({ restaurantId: restaurant.id, studentUserId, mealPlanId, startDate: new Date(`${startDate}T12:00:00`).toISOString() }) }, token); onSaved(); } catch (caught) { setError((caught as ApiError).message); } finally { setSaving(false); } };
  return <Modal title="Asignar pensión" eyebrow={restaurant.name} onClose={onClose}><p className="muted">Crea una suscripción para un estudiante ya registrado. La operación queda auditada con el restaurante seleccionado.</p><form onSubmit={save}><label>Estudiante *<select required value={studentUserId} onChange={(event) => setStudentUserId(event.target.value)}><option value="">Seleccionar estudiante…</option>{students.filter((student) => student.isActive).map((student) => <option key={student.id} value={student.id}>{student.fullName} · {student.email}</option>)}</select></label><label>Plan de pensión *<select required value={mealPlanId} onChange={(event) => setMealPlanId(event.target.value)}><option value="">Seleccionar plan…</option>{activePlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {plan.durationDays} días · {currencyFormatter.format(plan.price)}</option>)}</select></label><label>Fecha de inicio *<input required type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></label>{students.length === 0 && <p className="form-hint">Primero registra al menos un estudiante.</p>}{activePlans.length === 0 && <p className="form-hint">Primero crea al menos un plan activo.</p>}{error && <p className="form-error" role="alert">{error}</p>}<Actions onClose={onClose} saving={saving} action="Asignar suscripción" /></form></Modal>;
}
